import numpy as np
from data_reader import load_csv_data
...
def data_maker(DATA_PATH, DATA_SAVE_PATH, jets=False, verbose=False):
    ...
    - jets                      If true, the jets column will remain in the data set
    ...
    if jets:
        tX = np.hstack((tX[:,:4], tX[:,7:12], tX[:,13:23], tX[:,29:]))
